 ...
 * @param nh
 */
Monitor_l::Monitor_l(ros::NodeHandle* nh, int robot_num, int task_num)
{
  nh_ = nh;

  alliance_calculation_ = nh_->subscribe<alliance::to_monitor_l>("/monitor_bus", 1, &Monitor_l::monitorCallback, this);
  printTimer_ = nh_->createTimer(ros::Duration(0.2), &Monitor_l::timerCallBack, this);
 ...
 * @param msg
 */
void Monitor_l::monitorCallback(alliance::to_monitor_l msg)
{
  msgs_[msg.robot] = msg;
 ...
 * @param event
 */
void Monitor_l::timerCallBack(const ros::TimerEvent& event)
{
  printInfo();
}
  ...
  for (int i = 0; i < msgs_.size(); i++)  // for each robot
  {
    ...
    for (int j = 0; j < task_num_; j++)
    {
      ...
      std::cout << "\tAcquiescence \tMotivation \tActivity Sup."
                << "\tImpatience \tImpatience R. \tSensory Fb. \tTask Category" << std::endl;
      ...
      if (!msgs_[i].task_category.empty())
        std::cout << "\t" << float(msgs_[i].task_category[j]) << "\t";
