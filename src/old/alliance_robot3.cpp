...
{
  ROS_INFO("Iniciando node alliance_robot3...");
  ros::init(argc, argv, "alliance_robot3");
  ros::NodeHandle n;

  ros::Publisher task_pub = n.advertise<alliance::message>("TaskAllocation", 5);
  ros::Publisher cmd_vel_pub = n.advertise<geometry_msgs::Twist>("RosAria3/cmd_vel", 1);
  ros::Publisher motivation_pub = n.advertise<alliance::motivation>("Motivation_robot3", 1);

  ros::Subscriber tasksub = n.subscribe<alliance::message>("TaskAllocation", 5, TaskCallback);
  ros::Subscriber odomsub = n.subscribe<nav_msgs::Odometry>("RosAria3/odom", 10, OdomCallback);
  ros::Subscriber sonarsub = n.subscribe<sensor_msgs::PointCloud>("RosAria3/sonar", 10, SonarCallback);
