...
{
  ROS_INFO("Iniciando node alliance_robot1...");
  ros::init(argc, argv, "alliance_robot1");
  ros::NodeHandle n;

  ros::Publisher task_pub = n.advertise<alliance::message>("TaskAllocation", 5);
  ros::Publisher cmd_vel_pub = n.advertise<geometry_msgs::Twist>("RosAria1/cmd_vel", 1);
  ros::Publisher motivation_pub = n.advertise<alliance::motivation>("Motivation_robot1", 1);

  ros::Subscriber tasksub = n.subscribe<alliance::message>("TaskAllocation", 5, TaskCallback);
  ros::Subscriber odomsub = n.subscribe<nav_msgs::Odometry>("RosAria1/odom", 10, OdomCallback);
  ros::Subscriber sonarsub = n.subscribe<sensor_msgs::PointCloud>("RosAria1/sonar", 10, SonarCallback);
