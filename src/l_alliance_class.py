...
class Alliance:
    ...
    def __init__(self, theta, sensory_feedback, rho, tau, psi, phi,
                 task_num, robot_num, robot_id,
                 delta_slow, delta_fast, impatience, lambda_task,
                 boredom_rate, boredom_threshold, trained_times):
        ...
        self.trained_times = trained_times
        self.task_category = [0] * task_num
        self.learned_robot_influence = [0] * task_num
    ...
    def category_calc(self):
        ...
        for i in range(self.task_num):
            ...
            for j in range(self.robot_num):
                if j != self.robot_id:
                    comm_factor = comm_factor + self.comm[i][j]

            # print("LEN: " + str(len(self.trained_times)) + "\t" + str(len(self.trained_times[0])))
            if self.trained_times[i][self.robot_id] == min(self.trained_times[i]) and comm_factor == 0:
                self.task_category[i] = 1
            else:
                self.task_category[i] = 2

        # self.boredom_calc()

    def boredom_calc(self):
        suppression_product = 1
        for i in range(self.task_num):
            suppression_product = suppression_product * self.activity_suppression[i]

        self.boredom = (self.boredom + self.boredom_rate) * suppression_product
        if self.boredom > self.boredom_threshold:
            self.boredom = self.boredom_threshold

        for i in range(self.task_num):
            if self.boredom < self.boredom_threshold and self.task_category[i] == 2 and self.current_behaviour != i:
                self.learned_robot_influence[i] = 0
            else:
                self.learned_robot_influence[i] = 1
    ...
    def motivation_calc(self):
        ...
        for i in range(self.task_num):
            self.motivation[i] = (self.motivation_old[i] + self.impatience[i]) * \
                                 ...
                                 self.learned_robot_influence[i]
